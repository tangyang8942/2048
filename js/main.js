	...
	for ( var i = 0; i < 4; i++) {
		for ( var j = 0; j < 4; j++) {
			...
			if (nums[i][j] == 0) {
				...
				numberCell.css('top', getPosTop(i, j) + cellWidth*0.5+'px');
				numberCell.css('left', getPosLeft(i, j) + cellWidth*0.5+'px');
			} else {
			...
			$('.number-cell').css('line-height',cellWidth+'px')  //此处设置line-height时必须添加px，指定单位
		}
	}
}
